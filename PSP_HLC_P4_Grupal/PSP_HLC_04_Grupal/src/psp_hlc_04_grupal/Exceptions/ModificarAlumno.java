...
public class ModificarAlumno {
        //TO DO: Añadir un metodo que modifique la fecha de nacimiento del Alumno pasado como parametro
    private PreparedStatement ps;

    public Alumno obtenerAlumno(int numero) throws SQLException {
        String query = "SELECT * FROM Alumno WHERE numero = ?";
        ps = CommonActions.abrirConexion().prepareStatement(query);
        ps.setInt(1, numero);
        ...
        if (rs.next()) {
            alumno = new Alumno(
                ...
                new GregorianCalendar(rs.getDate("fechaNac").toLocalDate().getYear(),
